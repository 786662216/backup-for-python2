...
def jiangzemin(f):#判断时间到没到，到了就执行f()
    while True:
        ...
        schedtime = datetime.datetime(t.year, t.month, t.day, hour, hour, hour)  # 要执行的时间
        time.sleep(1)  # 循环执行的速度太快，虽然只有一秒但也会执行很多次,所以要等1s

        now = datetime.datetime.now()  # 返回值里面有微秒可把我坑惨了
        if now.date() == schedtime.date():  # 先判断日期相不相同
            if (now.hour == schedtime.hour) and (now.minute == schedtime.minute) and (
                        now.second == schedtime.second):  # 在判断时间相不相同，主要是为了避开微秒的比较
                f()
